...
=head1 SYNOPSIS
    ...
    my $meta = CPrAN::Meta->load_file('META.json');
...
Software distributions released to the CPrAN include a F<META.json> or, for
...
CPrAN::Meta provides a simple class to represent this distribution metadata (or
I<distmeta>), along with some helpful methods for interrogating that data.

The documentation below is only for the methods of the CPrAN::Meta object. For
information on the meaning of individual fields, consult the spec.

=cut

use Carp qw(carp croak);
use CPrAN::Meta::Feature;
use CPrAN::Meta::Prereqs;
use CPrAN::Meta::Converter;
use CPrAN::Meta::Validator;
use CPrAN::Meta::Parse::Meta 1.4414 ();

BEGIN { *_dclone = \&CPrAN::Meta::Converter::_dclone }
...
=method new
  ...
  my $meta = CPrAN::Meta->new($distmeta_struct, \%options);

Returns a valid CPrAN::Meta object or dies if the supplied metadata hash
...
fixable errors will be handled by CPrAN::Meta::Converter before validation.
...
sub _new {
  ...
  if ( $options->{lazy_validation} ) {
    ...
    my $cmc = CPrAN::Meta::Converter->new( $struct );
    $self = $cmc->convert( version => 2 ); # valid or dies
    return bless $self, $class;
  }
  else {
    # validate original struct
    my $cmv = CPrAN::Meta::Validator->new( $struct );
    unless ( $cmv->is_valid) {
...
=method create
  ...
  my $meta = CPrAN::Meta->create($distmeta_struct, \%options);

This is same as C<new()>, except that C<generated_by> and C<meta-spec> fields
will be generated if not provided.  This means the metadata structure is
assumed to otherwise follow the latest L<CPrAN::Meta::Spec>.
...
=method load_file
  ...
  my $meta = CPrAN::Meta->load_file($distmeta_file, \%options);

Given a pathname to a file containing metadata, this deserializes the file
according to its file suffix and constructs a new C<CPrAN::Meta> object, just
...
sub load_file {
  ...
  my $self;
  eval {
    my $struct = CPrAN::Meta::Parse::Meta->load_file( $file );
...
=method load_yaml_string
  ...
  my $meta = CPrAN::Meta->load_yaml_string($yaml, \%options);

This method returns a new CPrAN::Meta object using the first document in the
...
sub load_yaml_string {
  ...
  my $self;
  eval {
    my ($struct) = CPrAN::Meta::Parse::Meta->load_yaml_string( $yaml );
    $self = $class->_new($struct, $options);
  };
  croak($@) if $@;
  return $self;
...
=method load_json_string
  ...
  my $meta = CPrAN::Meta->load_json_string($json, \%options);

This method returns a new CPrAN::Meta object using the structure represented by
...
sub load_json_string {
  ...
  my $self;
  eval {
    my $struct = CPrAN::Meta::Parse::Meta->load_json_string( $json );
    $self = $class->_new($struct, $options);
  };
  croak($@) if $@;
  return $self;
...
=method load_string
  ...
  my $meta = CPrAN::Meta->load_string($string, \%options);

If you don't know if a string contains YAML or JSON, this method will use
L<CPrAN::Meta::Parse::Meta> to guess.  In other respects it is identical to
C<load_file()>.
...
sub load_string {
  ...
  my $self;
  eval {
    my $struct = CPrAN::Meta::Parse::Meta->load_string( $string );
    $self = $class->_new($struct, $options);
  };
  croak($@) if $@;
  return $self;
...
L<CPrAN::Meta::Converter> is used to generate an older metadata structure, which
is serialized to YAML. CPrAN::Meta::YAML is the default YAML backend. You may
...
This method returns a L<CPrAN::Meta::Prereqs> object describing all the
prereqs for the distribution.  If an arrayref of feature identifiers is given,
the prereqs for the identified features are merged together with the
distribution's core prereqs before the CPrAN::Meta::Prereqs object is returned.
...
sub effective_prereqs {
  ...
  my $prereq = CPrAN::Meta::Prereqs->new($self->prereqs);
...
This method returns a list of L<CPrAN::Meta::Feature> objects, one for each
...
sub features {
  ...
  my @features = map {; CPrAN::Meta::Feature->new($_ => $opt_f->{ $_ }) }
                 keys %$opt_f;
...
This method returns a L<CPrAN::Meta::Feature> object for the optional feature
...
sub feature {
  ...
  return CPrAN::Meta::Feature->new($ident, $f);
}
...
sub as_string {
  ...
  if ( $self->meta_spec_version ne $version ) {
    my $cmc = CPrAN::Meta::Converter->new( $self->as_struct );
  ...
  my ($data, $backend);
  if ( $version ge '2' ) {
    $backend = CPrAN::Meta::Parse::Meta->json_backend();
    local $struct->{x_serialization_backend} = sprintf '%s version %s',
      $backend, $backend->VERSION;
    $data = $backend->new->pretty->canonical->encode($struct);
  }
  else {
    $backend = CPrAN::Meta::Parse::Meta->yaml_backend();
    local $struct->{x_serialization_backend} = sprintf '%s version %s',
      $backend, $backend->VERSION;
...
# ABSTRACT: the distribution metadata for a CPrAN dist
...
=for :list
* L<CPrAN::Meta::Converter>
* L<CPrAN::Meta::Validator>
