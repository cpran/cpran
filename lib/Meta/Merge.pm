...
package CPrAN::Meta::Merge;

our $VERSION = '2.150006';

use Carp qw/croak/;
use Scalar::Util qw/blessed/;
use CPrAN::Meta::Converter 2.141170;
...
sub _optional_features {
  ...
  for my $key (keys %{$right}) {
    if (not exists $left->{$key}) {
      ...
      require CPrAN::Meta::Prereqs;
      $left->{$key}{prereqs} =
        CPrAN::Meta::Prereqs->new($left->{$key}{prereqs})
          ->with_merged_prereqs(CPrAN::Meta::Prereqs->new($right->{$key}{prereqs}))
          ->as_string_hash;
    }
  }
  return $left;
...
my %default = (
  ...
  prereqs           => sub {
    require CPrAN::Meta::Prereqs;
    my ($left, $right) = map { CPrAN::Meta::Prereqs->new($_) } @_[0,1];
...
sub merge {
  ...
  my $current = {};
  for my $next (@items) {
    if ( blessed($next) && $next->isa('CPrAN::Meta') ) {
      $next = $next->as_struct;
    }
    elsif ( ref($next) eq 'HASH' ) {
      my $cmc = CPrAN::Meta::Converter->new(
...
# ABSTRACT: Merging CPrAN Meta fragments

=head1 SYNOPSIS

 my $merger = CPrAN::Meta::Merge->new(default_version => "2");
...
This creates a CPrAN::Meta::Merge object. It takes one mandatory named
...
Merge all C<@fragments> together. It will accept both CPrAN::Meta objects and
