...
This module converts CPrAN Meta structures from one form to another.  The
...
use CPrAN::Meta::Requirements;
use CPrAN::Meta::Parse::Meta 1.4400 ();

# To help ExtUtils::MakeMaker bootstrap CPrAN::Meta::Requirements on perls
# before 5.10, we fall back to the EUMM bundled compatibility version module if
# that's the only thing available.  This shouldn't ever happen in a normal CPrAN
# install of CPrAN::Meta::Requirements, as version.pm will be picked up from
...
sub _dclone {
  ...
  my $json = CPrAN::Meta::Parse::Meta->json_backend()->new
      ->utf8
      ->allow_blessed
      ->convert_blessed;
...
my %known_specs = (
    '2'   => 'http://search.cpan.org/perldoc?CPrAN::Meta::Spec',
...
sub _version_map {
  ...
  if ( ref $element eq 'HASH' ) {
    # XXX turn this into CPrAN::Meta::Requirements with bad version hook
    # and then turn it back into a hash
    my $new_map = CPrAN::Meta::Requirements->new(
...
sub _get_build_requires {
  ...
  my $test_req  = CPrAN::Meta::Requirements->from_string_hash($test_h);
  my $build_req = CPrAN::Meta::Requirements->from_string_hash($build_h);
...
Optionally, you can provide a C<default_version> argument after C<$struct>:
  ...
  my $cmc = CPrAN::Meta::Converter->new( $struct, default_version => "1.4" );
...
Defaults to the latest version of the CPrAN Meta Spec.
...
For example, some badly structured META.yml files on CPrAN have prerequisite
...
sub convert {
  ...
  if ( $old_version == $new_version ) {
    ...
    unless ( $args->{is_fragment} ) {
      my $cmv = CPrAN::Meta::Validator->new( $converted );
      unless ( $cmv->is_valid ) {
        my $errs = join("\n", $cmv->errors);
...
of the CPrAN Meta Spec.  No validation is done on the result -- you must
...
# ABSTRACT: Convert CPrAN distribution metadata structures
